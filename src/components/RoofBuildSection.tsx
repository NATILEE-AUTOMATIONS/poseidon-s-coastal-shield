...
const RoofBuildSection: React.FC = () => {
  ...
  // Mobile: compressed to fit ALL animations including truck within 90% scroll
  ...
  // Mobile: compressed layer steps to fit truck animation within scroll range
  const mobileLayerStart = 0.10;
  const desktopLayerStart = 0.08;
  const mobileLayerStep = 0.038; // Compressed to fit all layers + dumpster + truck before 90%
  ...
  // Drip edge
  const dripEdgeMultiplier = isMobile ? 1.2 : 1.5;
  const dripEdgeEnd = deckingEnd + (layerStep * dripEdgeMultiplier);
  
  // Ice & water shield
  const iceWaterMultiplier = isMobile ? 1.2 : 1.5;
  ...
  // Calculate when all active layers end (truck now works on both mobile and desktop)
