...
const renderCard = ({ cardIndex, cardProgress, isNextCard = false }: CardRenderProps) => {
  let rotateY = 0;
  let translateX = 0;
  let scale = 1;
  let glowIntensity = 1;
  ...
  return (
    <div
      key={`card-${cardIndex}-${isNextCard ? 'next' : 'current'}`}
      className={`${isNextCard ? 'absolute inset-0' : 'relative'} px-8 py-6 rounded-2xl w-full max-w-md overflow-hidden`}
      style={{
...
const MobileStepCard: React.FC<MobileStepCardProps> = ({ 
  ...
  // Determine if we should show the next card (during exit phase overlap)
  const showNextCard = cardProgress >= exitStart && nextCardIndex !== currentCardIndex;
  
  // Calculate next card's progress: map current's 70-100% to next's 0-30%
  const nextCardProgress = showNextCard 
    ? ((cardProgress - exitStart) / (1 - exitStart)) * entryEnd 
    : 0;
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-md" style={{ transformStyle: 'preserve-3d' }}>
        {/* Current card */}
        {renderCard({ cardIndex: currentCardIndex, cardProgress })}
        
        {/* Next card - only during overlap period */}
